...
class Config:
    def __init__(self):
        ...
        self.anchor_box_scales = [64, 128, 256]   #[32, 64, 128]

        # Anchor box ratios. With three types [1, 1] [1, 2] [2, 1]
        self.anchor_box_ratios = [[1, 1], [1. / math.sqrt(2), 2. / math.sqrt(2)],
                                  [2. / math.sqrt(2), 1. / math.sqrt(2)]]

        # Size to resize the smallest side of the image
        # the number of image go to the ROI pooling layer.
...
# shape[:2] means grab just height and width by taking the first two elemets from shape [height, width, dimension]
def get_data(input_path):
    """
    Parse the data from annotation file.    Args: input_path: annotation file path
    Returns:
    ...
    with open(input_path, 'r') as f:
        ...
        for line in f:
            # Print process. "sys.stdout.write" similar with "print" function
            sys.stdout.write('\r' + 'idx=' + str(i))
            i += 1
            line_split = line.strip().split(',')
            # Make sure the info saved in annotation file matching the format (path_filename, x1, y1, x2, y2,class_name)
            # Note: One path_filename might has several classes (class_name).
...
def augment(img_data, config, augment=True):
    ...
    img = cv2.imread(img_data_aug['filepath'])  # shape of imread is (height, width, channel)
    if augment:
        rows, cols = img.shape[:2]
        if config.use_horizontal_flips and np.random.randint(0, 2) == 0:
            img = cv2.flip(img, 1)  # axs = 1
...
# calcaute IoU (intersection of union)
...
# perpare data for RPN including number of anchors and its size
def calc_rpn(C, img_data, width, height, resized_width, resized_height, img_length_calc_function):
    """(Important part!) Calculate all anchors for rpn
    ...
    """
    downscale = float(C.rpn_stride)  # 16 came from the CNN model
    anchor_sizes = C.anchor_box_scales  # 128, 256, 512
    anchor_ratios = C.anchor_box_ratios  # 1:1, 1:2*sqrt(2), 2*sqrt(2):1
    num_anchors = len(anchor_sizes) * len(anchor_ratios)  # 3x3=9

    # calculate the output map size based on the network architecture
    (output_height, output_width) = img_length_calc_function(resized_height, resized_width)

    # initialise empty output objectives
    y_rpn_overlap = np.zeros((output_height, output_width, num_anchors))  # one pixel has num_anchors's shape
    y_is_box_valid = np.zeros((output_height, output_width, num_anchors))  # just for those valid anchors
    y_rpn_regr = np.zeros((output_height, output_width, num_anchors * 4))  # regression output

    # This 6 lines are get the informaiton from the true bounding box
    num_bboxes = len(img_data['bboxes'])
    num_anchors_for_bbox = np.zeros(num_bboxes).astype(int)
    best_anchor_for_bbox = -1 * np.ones((num_bboxes, 4)).astype(int)  # the biggest value of box
    best_iou_for_bbox = np.zeros(num_bboxes).astype(np.float32)
    best_x_for_bbox = np.zeros((num_bboxes, 4)).astype(int)
    best_dx_for_bbox = np.zeros((num_bboxes, 4)).astype(np.float32)

    # Becaues of the resized the iamge, this 6 lines are correct its coordinates information (bounding box)
    ...
    # This whole part is pass each pixel in the feature map to prepare anchors that rpn need
                ...
                for jy in range(output_height):
                    ...
                    for bbox_num in range(num_bboxes):
                        ...
                        if curr_iou > best_iou_for_bbox[bbox_num] or curr_iou > C.rpn_max_overlap:
                            ...
                            # xa,ya are the center point of an anchor bbox (xa=downscale * (ix + 0.5); ya=downscale * (iy+0.5))
                            # w,h are the width and height of ground-truth bbox
                            # wa,ha are the width and height of an anchor bboxe
                            # tx = (x - xa) / wa
                            # ty = (y - ya) / ha
                            # tw = log(w / wa)
                            # th = log(h / ha)
                            # tx, ty, tw, th are the movement of literal and veteral
                        ...
                        if img_data['bboxes'][bbox_num]['class'] != 'bg':
                            # all GT boxes should be mapped to an anchor box, so we update "best" information
    ...
    # the highest IOU value from anchor and ground truth data.
    # we ensure that every bbox has at least one positive anchor
    ...
    # We also limit it to 256 regions in one mini-batch.
    num_regions = 256
    ...
    # output of classification [shape (1, width, height, 2*9)] and regression [shape ( , , ,4*9)]
    y_rpn_cls = np.concatenate([y_is_box_valid, y_rpn_overlap], axis=1)
    y_rpn_regr = np.concatenate([np.repeat(y_rpn_overlap, 4, axis=1), y_rpn_regr], axis=1)
    return np.copy(y_rpn_cls), np.copy(y_rpn_regr), num_pos


# generate the ground-truth anchors (training data)
def get_anchor_gt(all_img_data, C, img_length_calc_function, mode='train'):
    ...
    Returns:
        ...
        shape of x_imag: (1, width, height, 3(RGB))
        Y: [y_rpn_cls, y_rpn_regr]
        shape of Y: (1, feature.height, feature.width, 2*num_anchors) or (1, feature.height, feature.width, 4*num_anchors)
...
# pre-trained CNN model with Vgg-16 model
...
# define RPN newtwork layer
def rpn_layer(base_layers, num_anchors):
    """Create a rpn layer
        Step1: Pass through the feature map from base layer to a 3x3 512 channels convolutional layer
                Keep the feature map's size using padding = 'same'
        Step2: Pass the step1 to two (1,1) convolutional layer to replace the fully connected layer
                classification layer: num_anchors (9 in here) channels for 0, 1 sigmoid (or max) activation output
                regression layer: num_anchors*4 (36 in here) channels for computing the regression of bboxes with linear activation
    Args:
        base_layers: vgg in here (feature map from CNN model)
...
# build connection from rpn to roi. There are two steps need to do before rpn_to_roi
# "non_max_suppression_fast" and "apply_regr_np", which to control anchors from rpn to roi
...
def rpn_to_roi(rpn_layer, regr_layer, C, dim_ordering, use_regr=True, max_boxes=300, overlap_thresh=0.7):  # data_format
    """Convert rpn layer to roi bboxes  <The shape is (number of rois, 4)>
    Args: (num_anchors = 9)
    ...
    # Might be (4, 18, 25, 18) if resized image is 400 width and 300 height
    ...
    for anchor_size in anchor_sizes:
        for anchor_ratio in anchor_ratios:
            ...
            # X.shape = (18, 25)
            # Y.shape = (18, 25)
            X, Y = np.meshgrid(np.arange(cols), np.arange(rows))  # x is cols and y is rows grid
...
# generate the input of roipooling layer
def calc_iou(R, img_data, C, class_mapping):
    """Converts from (x1,y1,x2,y2) to (x,y,w,h) format
    output: X is the input of roipooling. Y1 and Y2 are the training data of final classificaiton and regression
    Args:
        R: bboxes, probs
    """
    ...
    x_roi = []  # the list of input of ROI with shape (x1, y1, w, h)
    y_class_num = []  # output class information
    y_class_regr_coords = []  # coords and label are the information of each class
    y_class_regr_label = []
...
class RoiPoolingConv(Layer):
    ...
    def __init__(self, pool_size, num_rois, **kwargs):
        ...
        super(RoiPoolingConv, self).__init__(**kwargs)  # **kwargs means sharing the same keyword arguments with parent
    ...
    def call(self, x, mask=None):
        ...
        # permute_dimensions function is similar to transpose
...
# Create Classifier layer (last step)
def classifier_layer(base_layers, input_rois, num_rois, nb_classes=3):
    """Create a classifier layer
    Args:
        base_layers: vgg
        input_rois: `(1,num_rois,4)` list of rois, with ordering (x,y,w,h) x and y are cooridate in the image and w and h are the width and height of the box
...
# define loss function for all four outputs
lambda_rpn_regr = 1.0
...
####code double checking using image exmaple####
# Get train data generator which generate X, Y, image_data and do some double check before model
...
if debug_num_pos == 0:
    ...
    color = (0, 255, 0)
    cv2.putText(img, 'gt bbox', (gt_x1, gt_y1 - 5), cv2.FONT_HERSHEY_DUPLEX, 0.5, color, 1)
...
# define the RPN, and built the neural networks using "Model" function
...
if not os.path.isfile(C.model_path):
    ...
    try:
        ...
        print('You need to manually download it from online including base and output weights')
    ...
    # If I have enough computer capability, I don't need this else part.
...
for epoch_num in range(num_epochs):
    ...
    print('Epoch {}/{}'.format(r_epochs + 1, total_epochs))  # print the summary of each batch
    r_epochs += 1
    while True:
        try:
            if len(rpn_accuracy_rpn_monitor) == epoch_length and C.verbose:
                ...
                if mean_overlapping_bboxes == 0:
                    print('No overlap from RPN with ground truth boxes. Check RPN settings or keep training.')
...
### This part is to visulate the training process.
